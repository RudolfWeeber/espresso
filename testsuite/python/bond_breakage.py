...
class BondBreakage(BondBreakageCommon, ut.TestCase):
    ...
    def test_00_interface(self):
        ...
        spec2 = BreakageSpec(breakage_length=1.2, action_type="revert_center_bond")
        spec4 = BreakageSpec(breakage_length=.2, action_type="revert_vs_bond")
...
class NetworkBreakage(BondBreakageCommon, ut.TestCase):
  ...
  def setUp(self):
    ...
    self.system.non_bonded_inter[0, 0].lennard_jones.set_params(sigma = 1.,
                                                           epsilon = 1., 
                                                           cutoff = 2**(1/6), 
                                                           shift = 'auto')
  ...
  def test_za_centerbonds(self):
    ...
    self.system.bond_breakage[harm._bond_id] = BreakageSpec(breakage_length=crit, action_type="revert_center_bond")
    self.system.integrator.run(1)

    bonds_dist = 0
    pairs = self.system.cell_system.get_pairs(crit, types=[0])
    for pair in pairs:
      dist = self.system.distance(self.system.part.by_id(pair[0]), self.system.part.by_id(pair[1]))
      if dist <= crit:
        bonds_dist += 1
 
    bonds_count = self.countBonds(pairs)
    np.testing.assert_equal(bonds_dist, bonds_count)
  
  def test_zz_vs_bonds(self):
    ...
    self.system.collision_detection.set_params(mode="off")
    self.system.bond_breakage[harm._bond_id] = BreakageSpec(breakage_length=crit_vs, action_type="revert_vs_bond")
    self.system.integrator.run(1)
    
    bonds_dist = 0
    pairs = self.system.cell_system.get_pairs(2**(1/6)*2/3, types=[1])
    
    for pair in pairs:
      r1 = self.system.part.by_id(pair[0]).vs_relative[0]
      r2 = self.system.part.by_id(pair[1]).vs_relative[0]
      dist = self.system.distance(self.system.part.by_id(r1), self.system.part.by_id(r2))
      dist_vs = self.system.distance(self.system.part.by_id(pair[0]), self.system.part.by_id(pair[1]))
      if dist_vs <= crit_vs:
        if dist <= crit:
          if dist > 0.0:
            bonds_dist += 1
