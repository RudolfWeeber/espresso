...
The propulsion speed is determined by the friction of a (Langevin)
...
the total system is net force-free at all times. The resulting flow-fields can thus not contain
...
To make make the force applier follow the swimmer and also update its orientation accordingly,
use the ``VirtualSitesRelative`` mechanism.
|es| provides a helper function :attr:`~espressomd.swimmer_helpers.add_dipole_particle` to set 
...
It lets you create pushers with the propulsion behind the swimmer and pullers with the 
propulsion in front.
