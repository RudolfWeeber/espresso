...
public:
  P3MFFT(boost::mpi::communicator comm, Utils::Vector3i global_mesh_size,
         Utils::Vector3i rs_local_ld_index, Utils::Vector3i rs_local_ur_index,
         Utils::Vector3i _mem_layout)
      : comm(comm), memory_layout(_mem_layout), global_mesh(global_mesh_size),
  ...
  void set_preferred_kspace_decomposition() {
    ...
    if (proc_grid[1] == 1 and proc_grid[0] == 1 and proc_grid[2] % 4 == 0 and
        local_size[1] % 2 == 0) {
      proc_grid[2] /= 2;
      proc_grid[1] *= 2;
    }
