...
void CellStructure::set_index_map() {
  ...
  Kokkos::fence();
  // Stamp each local cell with its AoSoA start index; reset ghost cells.
  std::size_t aosoa_offset = 0;
  for (auto *c : m_decomposition->local_cells()) {
    c->set_aosoa_offset(aosoa_offset);
    aosoa_offset += c->particles().size();
  }
  for (auto *c : m_decomposition->ghost_cells())
    c->set_aosoa_offset(Cell::no_aosoa_slot);
