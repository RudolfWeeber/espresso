...
std::vector<double>
LBWalberlaImplSingleComponent<FloatType, Architecture>::get_slice_velocity(
...
template <typename FloatType, lbmpy::Arch Architecture>
void LBWalberlaImplSingleComponent<FloatType, Architecture>::set_slice_velocity(
...
template <typename FloatType, lbmpy::Arch Architecture>
std::vector<double> LBWalberlaImplSingleComponent<FloatType, Architecture>::
...
template <typename FloatType, lbmpy::Arch Architecture>
std::vector<double>
LBWalberlaImplSingleComponent<FloatType, Architecture>::get_slice_population(
    Utils::Vector3i const &lower_corner,
    Utils::Vector3i const &upper_corner) const {
  std::vector<double> out;
  for_each_block_in_slice(
      ...
      [&](auto const &block, auto const &bci, auto const &ci,
          auto const &block_offset) {
        if (out.empty())
          out.resize(this->stencil_size() * ci.numCells());
...
template <typename FloatType, lbmpy::Arch Architecture>
void LBWalberlaImplSingleComponent<FloatType, Architecture>::
    set_slice_population(Utils::Vector3i const &lower_corner,
                         Utils::Vector3i const &upper_corner,
  ...
  for_each_block_in_slice(
      ...
      [&](auto &block, auto const &bci, auto const &ci,
        ...
        auto kernel = [&values_a, &population, this](unsigned const block_index,
                                                     unsigned const local_index,
                                                     Utils::Vector3i const &) {
          for (uint_t f = 0u; f < this->stencil_size(); ++f) {
            values_a[this->stencil_size() * block_index + f] =
...
template <typename FloatType, lbmpy::Arch Architecture>
std::vector<double>
LBWalberlaImplSingleComponent<FloatType, Architecture>::get_slice_density(
    Utils::Vector3i const &lower_corner,
    Utils::Vector3i const &upper_corner) const {
  std::vector<double> out;
  for_each_block_in_slice(
...
template <typename FloatType, lbmpy::Arch Architecture>
void LBWalberlaImplSingleComponent<FloatType, Architecture>::set_slice_density(
    Utils::Vector3i const &lower_corner, Utils::Vector3i const &upper_corner,
