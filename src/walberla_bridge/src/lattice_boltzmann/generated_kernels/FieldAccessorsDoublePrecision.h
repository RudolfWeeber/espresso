...
inline void set(GhostLayerField<double, uint_t{19u}> *pdf_field,
                double const rho_in, double const density, Cell const &cell) {
  const double &xyz0 = pdf_field->get(cell, uint_t{0u});
  const double f_0 = pdf_field->getF(&xyz0, uint_t{0u});
  const double f_1 = pdf_field->getF(&xyz0, uint_t{1u});
  const double f_2 = pdf_field->getF(&xyz0, uint_t{2u});
...
get(GhostLayerField<double, uint_t{19u}> const *pdf_field, double const density,
    CellInterval const &ci) {
  std::vector<double> out;
...
inline void set(GhostLayerField<double, uint_t{19u}> *pdf_field,
                std::vector<double> const &values, double const density,
                CellInterval const &ci) {
  ...
  for (auto x = ci.xMin(); x <= ci.xMax(); ++x) {
    for (auto y = ci.yMin(); y <= ci.yMax(); ++y) {
      for (auto z = ci.zMin(); z <= ci.zMax(); ++z) {
        ...
        Equilibrium::set(pdf_field, velocity, *values_it * density_inv,
                         Cell{x, y, z});
        ++values_it;
      }
    }
...
inline void set(GhostLayerField<double, uint_t{19u}> const *pdf_field,
                ...
                Vector3<double> const &force, double const density,
                Cell const &cell) {
  ...
  const double momdensity_2 =
      f_11 + f_14 - f_15 - f_16 - f_17 - f_18 - f_6 + vel2Term;
  const double rho = delta_rho + 1;
  const double md_0 = momdensity_0 + force[0u] * 0.50000000000000000 / density;
  const double md_1 = momdensity_1 + force[1u] * 0.50000000000000000 / density;
  const double md_2 = momdensity_2 + force[2u] * 0.50000000000000000 / density;
  ...
  for (auto x = ci.xMin(); x <= ci.xMax(); ++x) {
    for (auto y = ci.yMin(); y <= ci.yMax(); ++y) {
      for (auto z = ci.zMin(); z <= ci.zMax(); ++z) {
        ...
        const double rho = delta_rho + 1;
        const double md_0 =
            momdensity_0 + force[0u] * 0.50000000000000000 / density;
        const double md_1 =
            momdensity_1 + force[1u] * 0.50000000000000000 / density;
        const double md_2 =
            momdensity_2 + force[2u] * 0.50000000000000000 / density;
        auto const rho_inv = double{1} / rho;

        force_field->getF(&laf_xyz0, uint_t{0u}) = force[0u] * density_inv;
        force_field->getF(&laf_xyz0, uint_t{1u}) = force[1u] * density_inv;
        force_field->getF(&laf_xyz0, uint_t{2u}) = force[2u] * density_inv;
...
inline auto get(GhostLayerField<double, uint_t{19u}> const *pdf_field,
  ...
  const int y = cell[1];
  const int z = cell[2];
  const double f_1 = pdf_field->get(0 + x, -1 + y, 0 + z, uint_t{1u});
  const double f_2 = pdf_field->get(0 + x, 1 + y, 0 + z, uint_t{2u});
  const double f_3 = pdf_field->get(1 + x, 0 + y, 0 + z, uint_t{3u});
  const double f_4 = pdf_field->get(-1 + x, 0 + y, 0 + z, uint_t{4u});
  const double f_5 = pdf_field->get(0 + x, 0 + y, -1 + z, uint_t{5u});
  const double f_6 = pdf_field->get(0 + x, 0 + y, 1 + z, uint_t{6u});
  const double f_7 = pdf_field->get(1 + x, -1 + y, 0 + z, uint_t{7u});
  const double f_8 = pdf_field->get(-1 + x, -1 + y, 0 + z, uint_t{8u});
  const double f_9 = pdf_field->get(1 + x, 1 + y, 0 + z, uint_t{9u});
  const double f_10 = pdf_field->get(-1 + x, 1 + y, 0 + z, uint_t{10u});
  const double f_11 = pdf_field->get(0 + x, -1 + y, -1 + z, uint_t{11u});
  const double f_12 = pdf_field->get(0 + x, 1 + y, -1 + z, uint_t{12u});
  const double f_13 = pdf_field->get(1 + x, 0 + y, -1 + z, uint_t{13u});
  const double f_14 = pdf_field->get(-1 + x, 0 + y, -1 + z, uint_t{14u});
  const double f_15 = pdf_field->get(0 + x, -1 + y, 1 + z, uint_t{15u});
  const double f_16 = pdf_field->get(0 + x, 1 + y, 1 + z, uint_t{16u});
  const double f_17 = pdf_field->get(1 + x, 0 + y, 1 + z, uint_t{17u});
  const double f_18 = pdf_field->get(-1 + x, 0 + y, 1 + z, uint_t{18u});
  ...
  for (auto x = ci.xMin(); x <= ci.xMax(); ++x) {
    for (auto y = ci.yMin(); y <= ci.yMax(); ++y) {
      for (auto z = ci.zMin(); z <= ci.zMax(); ++z) {
        ...
        const double f_17 = pdf_field->get(1 + x, 0 + y, 1 + z, uint_t{17u});
        const double f_18 = pdf_field->get(-1 + x, 0 + y, 1 + z, uint_t{18u});
        const double p_0 = f_10 + f_13 + f_14 + f_17 + f_18 + f_3 + f_4 + f_7 +
                           f_8 + f_9 + 0.33333333333333333;
        const double p_1 = -f_10 - f_7 + f_8 + f_9;
        const double p_2 = -f_13 + f_14 + f_17 - f_18;
        const double p_3 = -f_10 - f_7 + f_8 + f_9;
        const double p_4 = f_1 + f_10 + f_11 + f_12 + f_15 + f_16 + f_2 + f_7 +
                           f_8 + f_9 + 0.33333333333333333;
        const double p_5 = f_11 - f_12 - f_15 + f_16;
        const double p_6 = -f_13 + f_14 + f_17 - f_18;
        const double p_7 = f_11 - f_12 - f_15 + f_16;
