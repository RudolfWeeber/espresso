...
LBWalberlaImplSingleComponent<FloatType, Architecture>::get_node_velocity(
...
template <typename FloatType, lbmpy::Arch Architecture>
bool LBWalberlaImplSingleComponent<FloatType, Architecture>::set_node_velocity(
...
LBWalberlaImplSingleComponent<FloatType, Architecture>::get_node_population(
    Utils::Vector3i const &node, bool consider_ghosts) const {
  assert(not(consider_ghosts and m_pending_ghost_comm.test(GhostComm::PDF)));
  auto bc = get_block_and_cell(get_lattice(), node, consider_ghosts);
  if (!bc)
...
bool LBWalberlaImplSingleComponent<FloatType, Architecture>::
    set_node_population(Utils::Vector3i const &node,
...
LBWalberlaImplSingleComponent<FloatType, Architecture>::
    get_node_pressure_tensor(Utils::Vector3i const &node) const {
  auto bc = get_block_and_cell(get_lattice(), node, false);
  if (!bc)
    return std::nullopt;
