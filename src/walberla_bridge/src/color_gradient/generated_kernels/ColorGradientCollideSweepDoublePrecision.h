...
//
//! \\file ColorGradientCollideSweepDoublePrecision.h
//! \\author pystencils
//======================================================================================================================
...
{
public:
   ColorGradientCollideSweepDoublePrecision( BlockDataID force_aID_, BlockDataID force_bID_, BlockDataID pdfs_aID_, BlockDataID pdfs_bID_, BlockDataID phasefieldID_, BlockDataID rho_aID_, BlockDataID rho_bID_, BlockDataID velocityID_, double omega_even_a, double omega_even_b, double omega_odd_a, double omega_odd_b, double omega_shear_a, double omega_shear_b )
     : force_aID(force_aID_), force_bID(force_bID_), pdfs_aID(pdfs_aID_), pdfs_bID(pdfs_bID_), phasefieldID(phasefieldID_), rho_aID(rho_aID_), rho_bID(rho_bID_), velocityID(velocityID_), omega_even_a_(omega_even_a), omega_even_b_(omega_even_b), omega_odd_a_(omega_odd_a), omega_odd_b_(omega_odd_b), omega_shear_a_(omega_shear_a), omega_shear_b_(omega_shear_b)
   {}
   ...
   static std::function<void (IBlock *)> getSweep(const shared_ptr<ColorGradientCollideSweepDoublePrecision> & kernel)
   {
     return [kernel]
            (IBlock * b)
            { kernel->run(b); };
   ...
   static std::function<void (IBlock*)> getSweepOnCellInterval(const shared_ptr<ColorGradientCollideSweepDoublePrecision> & kernel, const shared_ptr<StructuredBlockStorage> & blocks, const CellInterval & globalCellInterval, cell_idx_t ghostLayers=1)
   {
