...
def calc_quaternions_from_angles(polar_angle, azimuthal_angle):
    """
    Takes the angles from a spherical coordinate representation of a vector and 
    returns the quaternion that corresponds to that direction.
    ...
    q1w = np.cos(polar_angle / 2.0)
    q1x = 0
    q1y = np.sin(polar_angle / 2.0)
    q1z = 0

    q2w = np.cos(azimuthal_angle / 2.0)
    q2x = 0
    q2y = 0
    q2z = np.sin(azimuthal_angle / 2.0)
