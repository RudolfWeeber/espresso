...
test_values = {prop: getattr(test_particle, prop) for prop in all_props_write}
test_values_slice = {
    prop: np.array([getattr(test_particle, prop)] * n_part) for prop in all_props_write
}
...
def benchmark_property_access(properties, access_func, time_scale=1e6, time_unit="µs"):
    ...
    for prop in properties:
        avg, std = benchmark_operation(lambda: access_func(prop), args.n_iter)
        print(
            f"  {prop}: {avg * time_scale:.3f} {time_unit} ± {std *
...
benchmark_property_access(
    all_props_read, lambda prop: getattr(test_particle, prop))
...
def write_property(prop):
    setattr(test_particle, prop, test_values[prop])
...
def benchmark_slice_access(
    properties, access_func, description, time_scale=1e3, time_unit="ms"
):
...
def write_slice_property(prop):
    setattr(system.part.all(), prop, test_values_slice[prop])
...
def write_cached_slice(slice_obj, prop):
    setattr(slice_obj, prop, test_values_slice[prop])
